...
usage() {
  cat <<'HELP'
Usage:
  parallel-vs-single.sh anonymize --pair-id <id> --parallel-room <room> --single-room <room> [--study-dir DIR] [--force]
...
rand8() { printf '%s:%s:%s\n' "$1" "$2" "$RANDOM" | sha256sum | awk '{print substr($1,1,8)}'; }
...
case "$cmd" in
  ...
  anonymize)
    pair_id=""; parallel_room=""; single_room=""; force=0
    while [ $# -gt 0 ]; do
      case "$1" in
    ...
    for mode in parallel single; do
      ...
      row=$(jq -c --arg r "$src_room" 'select(.room==$r)' "$JOURNAL" | tail -1)
      ...
      if [ -e "$dst_dir" ] && [ "$force" != "1" ]; then
        die "anonymized room already exists: $anon_room (use --force to replace)"
      fi
      rm -rf "$dst_dir"
      ...
      if grep -Eiq 'parallel-subagent|single-context|Task tool|Codex|Cursor|Claude Code' "$dst_dir/log.jsonl"; then
        echo "WARN: $anon_room transcript may contain mode-revealing text; inspect before judging" >&2
      fi

      jq -c --arg room "$anon_room" --arg ts "$(now_utc)" \
        '. + {ts:$ts, room:$room, judge_blinded:false, judge_blinded_catch:null,
      ...
      jq -cn --arg pair_id "$pair_id" --arg mode "$mode" --arg original_room "$src_room" \
        ...
        '{pair_id:$pair_id, mode:$mode, original_room:$original_room,
          anon_room:$anon_room, created_at:$created_at}' >> "$map_file"
    ...
    jq -nr --slurpfile map "$map_file" --slurpfile rows "$JOURNAL" '
      ...
      | "parallel agreement: \($pa)/\($pn)",
        ...
        ([$runs | group_by(.pair_id)[]
          | if length == 2 and all(.agreement != null) then
              {pair_id:.[0].pair_id,
