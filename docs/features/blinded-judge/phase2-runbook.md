...
- `status`: `ready`, `ambiguous-room`, `missing-journal`, `no-synthesis`, `missing-artifact`, `missing-transcript`, `no-positions`, or `judged`.
  ...
  - `missing-journal` means a transcript/artifact exists but no self-report row exists; run or reconstruct `journal.sh append` before judging.
