...
> Qualitative shape-of-disagreement readout from the first 5 dual-judged councils. Closes #48; closes #1 via the calibration-not-code criterion. **NOT** the quantitative band decision — that's Phase 2 + DRI-decision-`<date>`.md.
...
1. **Expected band**: at 5/5 cross-family agreement in Phase 1, the prior for Phase 2 cross-family agreement rate is biased toward the high band (>90%). But Phase 2's sample will:
   ...
   With those changes, **the agreement rate is more likely to land in the 70-90% band than to hold at 100%**. The Phase 1 number is an upper bound on Claude/Gemini cross-family agreement, not a prior for the full Phase 2 space.
...
- ❌ Does not close issue #1 via code. Issue #1 closes via this writeup (Phase 1 + DRI sign-off below).
...
> Phase 1 ran with three structural caveats: (a) 3 of 5 Claude judge-a calls were contaminated by in-session context, (b) 4 of 5 rooms had no synthesis block, muting the rubric's dissent-erasure arm, and (c) Phase 1 was all-cross-family (Claude judge-a vs Gemini judge-b) with zero same-family and zero 3rd-family data. The 5/5 agreement rate is therefore an **upper bound on Claude/Gemini cross-family robustness**, not a calibrated prior for the full Phase 2 space. The most informative single observation is the disagreement between LLM in-session Claude judges and fresh-chat Claude judges on `council-cache-prd` (verdicts flipped from false to true) — directly demonstrating that judge-context matters and validating the fresh-chat protocol. Phase 2 should front-load same-family and 3rd-family runs to fill the Phase 1 sampling gap before pooling. — @Zhachory1, 2026-06-16
...
- **Closes:** #48, #1 (via #20's calibration-not-code criterion)
